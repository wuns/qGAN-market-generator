...
\begin{frame}{From noise to market scenario: hybrid pipeline}
    ...
    \begin{itemize}
        \item[$\rightarrow$] Discriminator unchanged from the classical baseline
        \item[$\rightarrow$] Only the generator differs
    \end{itemize}
...
\begin{frame}{The variational quantum circuit}
    ...
    \begin{column}{.55\textwidth}
        ...
        \begin{itemize}
            \item \textbf{Initial state} $|\mathbf{0}\rangle = |0\rangle^{\otimes n_q}$
    ...
    \begin{column}{.43\textwidth}
        \begin{block}{Why this design?}
            \begin{itemize}
                \item HEA: nearest-neighbor
                CNOTs only
                \item $RY$-only rotations mitigate barren
                plateaus relative to richer ansätze~\cite{McClean2018BarrenPlateaus}
                \item Local Pauli-Z measurements bound gradient variance~\cite{Cerezo2021LocalCost}
                \item $L=3$: enough depth for entanglement to spread; deeper risks barren plateaus
            \end{itemize}
        \end{block}
    \end{column}
    \end{columns}
