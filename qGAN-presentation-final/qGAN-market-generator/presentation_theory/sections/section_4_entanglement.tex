...
\begin{frame}{Two routes to correlated samples}
    ...
    \begin{itemize}
        \item {\normalsize Hypothesis: the hybrid generator is \textbf{more parameter-efficient} for joint distributions.}             
        \item {\normalsize We test this hypothesis \textit{empirically}.}    
    \end{itemize} 
...
\begin{frame}{What we (do not) claim}
    \begin{columns}[T]
    \begin{column}{.48\textwidth}
        \textbf{We claim, theoretically, that... }
        \begin{itemize}
    ...
    \hfill
    \begin{column}{.48\textwidth}
        \textbf{We do \textit{not} claim that...}
        \begin{itemize}
            \item ... classical NNs cannot represent
            these distributions (they can, given enough
            capacity)
            \item ... parameter-efficiency manifests in
            every regime
            \item ... entanglement is the \emph{only}
